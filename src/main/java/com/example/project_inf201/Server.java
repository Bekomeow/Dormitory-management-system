            ...
            try {
                ...
                switch (message[1]) {
                    ...
                    case "check student password" : {
                        String[] data = message[2].split(", ");
                        String email = data[0];
                        String password = DataHashing.makeHashPassword(data[1], "SHA-256");
                        boolean check = database.checkStudentPassword(email, password);
                        if(check){
                            ServerConnection.sendMessage(socket, "true");
                        }
                        else{
                    ...
                    case "check university password" : {
                        ...
                        String email = data[0];
                        String password = DataHashing.makeHashPassword(data[1], "SHA-256");
                        boolean check = database.checkUniversityPassword(email, password);
                        if(check){
                            ServerConnection.sendMessage(socket, "true");
                        }
                        else{
